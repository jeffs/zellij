...
#[test]
fn test_ctrl_scroll_does_not_resize_pane() {
    let size = Size {
        cols: 120,
        rows: 20,
    };
    ...
    // each direction is checked on its own: an up/down pair would resize and
    // then un-resize, hiding the very behaviour this test exists to catch
    ...
    for (direction, event) in [
        ...
        tab.handle_mouse_event(&event, client_id).unwrap();

        // without this the render below only serializes cells that changed,
        ...
        assert_eq!(
            ...
            "ctrl+scroll {direction} must not resize panes"
        );
    }
}
